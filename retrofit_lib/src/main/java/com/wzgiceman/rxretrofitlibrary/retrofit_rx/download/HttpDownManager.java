...
public class HttpDownManager {
    /*记录下载数据*/
    private Set<DownInfoEntity> downInfos;
    /*回调sub队列*/
    ...
    public static final String TAG = HttpDownManager.class.getSimpleName();
     ...
     * 开始下载
     */
    public void startDown(final DownInfoEntity info) {
        /*正在下载不处理*/
        if (info == null || subMap.get(info.getUrl()) != null) {
            subMap.get(info.getUrl()).setDownInfo(info);
            return;
        }
        Log.d("TAG","下载文件   "+info.toString());
        /*添加回调处理类*/
        ...
        httpService.download("bytes=" + info.getReadLength() + "-", info.getUrl())
                ...
                /*读取下载写入文件*/
                .map(new Func1<ResponseBody, DownInfoEntity>() {
                    @Override
                    public DownInfoEntity call(ResponseBody responseBody) {
     ...
     * 停止下载
     */
    public void stopDown(DownInfoEntity info) {
        if (info == null) return;
     ...
     * @param info
     */
    public void pause(DownInfoEntity info) {
        if (info == null) return;
    ...
    public void stopAllDown() {
        for (DownInfoEntity downInfo : downInfos) {
            stopDown(downInfo);
        }
        subMap.clear();
        downInfos.clear();
     ...
     * @return
     */
    public Set<DownInfoEntity> getDownInfos() {
        return downInfos;
    }
     ...
     * @param info
     */
    public void remove(DownInfoEntity info) {
        subMap.remove(info.getUrl());
     ...
     * @throws IOException
     */
    public void writeCaches(ResponseBody responseBody, File file, DownInfoEntity info) {
        try {
