...
public class ProgressSubscriber<T> extends Subscriber<T> {
    ...
    public void onStart() {
        ...
        if (api.isCache() && AppUtil.isNetworkAvailable(RxRetrofitApp.getApplication())) {
             /*获取缓存数据*/
            CookieResulte cookieResulte = AppDataBase.getInstance().getCookieDao().queryCookieBy(api.getUrl());
        ...
        if (api.isCache()) {
            Observable.just(api.getUrl()).subscribe(new Subscriber<String>() {
                ...
                public void onNext(String s) {
                    /*获取缓存数据*/
                    CookieResulte cookieResulte = AppDataBase.getInstance().getCookieDao().queryCookieBy(s);
                    if (cookieResulte == null) {
                    ...
                    if (time < api.getCookieNoNetWorkTime()) {
                        ...
                        AppDataBase.getInstance().getCookieDao().deleteCookie(cookieResulte);
                        throw new HttpTimeException("网络错误");
                    }
                }
            });
